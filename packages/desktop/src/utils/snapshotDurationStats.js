import { shouldExcludePerAppTodayItem } from '@time-manger/shared'
...
export function getSnapshotDurationStats(snapshot) {
  ...
  const todayTotalMs = (snapshot.perAppToday || []).reduce((total, item) => {
    if (shouldExcludePerAppTodayItem(item)) return total
    return total + (item.durationMs || 0)
  }, 0)
