...
// 1. 作用：连续使用达到该时长后，`PetBubble` 使用鲜红主题（`long-work`）；同阈值用于宠物 `mood`（`warn`），见 `src/App.jsx`。
// 2. 使用：`src/components/PetBubble/PetBubble.jsx`；`src/App.jsx`（`petView.mood`）。
export const LONG_WORK_CONTINUOUS_MS = 50 * 60 * 1000

// 1. 作用：今日累计空闲休息达到该时长后，气泡文案为「休息完成…」并可走草绿主题（与 `isOnBreak` 等分支共同决定最终样式）；同阈值用于宠物 `mood`（`happy`）。
// 2. 使用：`src/components/PetBubble/PetBubble.jsx`；`src/App.jsx`（`petView.mood`）。
